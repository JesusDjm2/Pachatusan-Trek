...
@section('contenido')
    <div class="wrapper">
                        ...
                        <div class="header-wrapper">
                            ...
                            <nav class="main-nav">
                                <ul class="one-page-menu">
                                    ...
                                    <li id="display">
                                        @if ($enblog)
                                            <a href="{{ route('enblog.show', $enblog->slug) }}">
        ...
        <div class="blogContenedor">
            <img src="{{ asset($blog->imgFull) }}" alt="{{ $blog->nombre }}" class="fullscreen-img zoom-infinite">
            <div class="content-overlay">
                <h1>{{ $blog->nombre }}</h1>
                <p>{{ $blog->descripcionCorta }}</p>
                @foreach ($blog->tags as $tag)
                    <a href="{{ route('estag.show', $tag->slug) }}" class="tags">{{ $tag->nombre }}</a>
                @endforeach
            </div>
        </div>
    </div>
            ...
            <div class="col-lg-3" style="position: sticky;top:5em">
                ...
                @foreach ($blogs as $item)
                    <div class="blogsRelacionados">
                        <div class="div">
                            <a href="{{ route('esblog.show', $item->slug) }}">
                                <img src="{{ asset($item->imgThumb) }}" alt="{{ $item->nombre }}" loading="lazy">
                            </a>
                            <a href="{{ route('esblog.show', $item->slug) }}">
                                <h3>{{ $item->nombre }}</h3>
                            </a>
                ...
                <div class="tags">
                    ...
                    <div class="div">
                        @foreach ($blog->tags as $tag)
                            <a href="{{ route('estag.show', $tag->slug) }}" class="enlaceTags">{{ $tag->nombre }}</a>
                        @endforeach
                    </div>
                </div>
            </div>
